...
//                              &mCurrentImageIndex);
        ...
        viewport.x = 0.0f;
        viewport.y = 0.0f;
        viewport.width = (float) mCtx->windowExtents.width;
        viewport.height = (float) mCtx->windowExtents.height;
        viewport.minDepth = 0.0f;
        viewport.maxDepth = 1.0f;

        VkRect2D scissor{};
        scissor.offset = {0, 0};
        scissor.extent = mCtx->windowExtents;
...
//        vkQueuePresentKHR(mCtx->presentationQueue, &presentInfoKhr);
    ...
    void ShadowMap::ReCreateResourcesForWindowResize() {
